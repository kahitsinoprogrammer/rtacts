...
const getVoucherId = (voucher: JournalVoucher): string =>
...
export default function JVlist() {
  ...
  >({});
  const [rejectTarget, setRejectTarget] = useState<JournalVoucher | null>(null);
  ...
  const updateVoucherStatus = async (
    ...
    try {
      ...
      const response = await fetch(
        `http://localhost:8080/journal-vouchers/${voucherId}/status`,
        {
          method: "PATCH",
          headers: {
      ...
      if (!response.ok) {
        throw new Error(
          await getErrorMessage(
            response,
            `Failed to ${status.toLowerCase()} journal voucher`,
          ),
        );
      }
  ...
  const downloadVoucherExcel = async (voucher: JournalVoucher) => {
    const voucherId = getVoucherId(voucher);
    if (!voucherId) return;
    ...
    try {
      ...
      const response = await fetch(
        `http://localhost:8080/journal-vouchers/${voucherId}/excel`,
        {
          method: "GET",
          credentials: "include",
        },
      ...
      if (!response.ok) {
        throw new Error(
          await getErrorMessage(response, "Failed to download journal voucher"),
        );
      }
      ...
      setError(
        err instanceof Error
          ? err.message
          : "Failed to download journal voucher",
      );
    } finally {
  ...
  useEffect(() => {
    const fetchVouchers = async () => {
      try {
        ...
        const vouchersRes = await fetch("http://localhost:8080/journal-vouchers", {
          method: "GET",
          credentials: "include",
        });

        if (!vouchersRes.ok) {
          throw new Error("Failed to load journal vouchers");
        }
        ...
        setError(
          err instanceof Error ? err.message : "Failed to load journal vouchers",
        );
        setVouchers([]);
      } finally {
        setIsLoading(false);
    ...
    <div className="w-full p-4 sm:p-6">
      <div className="mb-5 rounded-xl border border-slate-200 bg-white p-4 shadow-sm">
        <div className="mb-4 flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
          <div>
            <h2 className="text-xl font-semibold text-slate-900">Journal Voucher List</h2>
          ...
          <input
            ...
            placeholder="Search by JV ID, supplier, email, status, or remarks..."
      ...
      {isLoading && <p className="text-sm text-slate-600">Loading journal vouchers...</p>}
      {!isLoading && error && (
            ...
            return (
              ...
              >
                <div className="mb-3 flex items-start justify-between gap-3">
                  <div>
                    <p className="text-xs uppercase tracking-wide text-slate-500">JV ID</p>
      ...
      >
        <div className="space-y-4">
          <p className="text-sm text-slate-600">
            Add the reason for rejecting this journal voucher. This will be saved and shown in the list.
          </p>
